...
def _get_embedder():
    ...
    if _embedder is None:
        ...
        _embedder = FlagModel(EMBED_MODEL, use_fp16=True)
    return _embedder
...
def retrieve_context(query: str, top_k: int = 10, rerank_top_n: int = 3) -> str:
    """
    Embed the query → fetch top_k candidates → rerank → return top rerank_top_n
...
def clear_collection():
    """Wipe all stored data (call before loading a new PDF)."""
    global _collection
